...
WrapInFunction.SettingsDescriptor = {
	Iterations = {
		...
		type = "number",
		default = 1,  -- Ubah dari 0 menjadi 1
		min = 0,
		max = nil,
	}
}
...
function WrapInFunction:apply(ast)
	-- Hapus atau comment pengecekan ini, atau biarkan tetap ada
	-- Tapi pastikan Iterations > 0
	if not self.Iterations or self.Iterations == 0 then
		return ast;
	end
